...
# INPUT_REVIEWDOG_FLAGS is quoted to prevent shell metacharacter injection.
# The ${VAR:+"$VAR"} form drops the argument entirely when the variable is
# empty (avoiding a spurious empty-string argument) and expands it as a
# double-quoted word when non-empty (preventing word-splitting / injection).
reviewdog -f=checkstyle \
    ...
    -level="${INPUT_LEVEL}" \
    ${INPUT_REVIEWDOG_FLAGS:+"${INPUT_REVIEWDOG_FLAGS}"} < "$cs_output" || rd_exit=$?
rd_exit=${rd_exit:-0}
